...
let completion_positions ~eager (doc, positions) : string -> unit =
  ...
  let completions_at_position ?key (position: Position.t) =
    ...
    match LSP.Request.completion ~eager server params with
    | None ->
...
let div_and_section = extract_position_markers {cobol|
...
let%expect_test "division-and-section-completion" =
  let end_with_postproc = completion_positions ~eager:false div_and_section in
...
let%expect_test "eager-division-and-section-completion" =
  let end_with_postproc = completion_positions ~eager:true div_and_section in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
    __rootdir__/prog.cob:2.8:
...
let datadiv = extract_position_markers {cobol|
...
let%expect_test "datadiv-completion" =
  let end_with_postproc = completion_positions ~eager:false datadiv in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
    ...
    (line 5, character 11):
    List of completions (74 entries): [VARYING;VALUES;VALUE;VAL-STATUS;USAGE;TYPEDEF;TYPE;TRAILING;SYNCHRONIZED;SIGN;SELECT;SAME;REDEFINES;PROPERTY;PROGRAM-POINTER;PROCEDURE-POINTER;PRESENT;POINTER;PICTURE;.;PACKED-DECIMAL;OCCURS;OBJECT;NATIONAL;LEADING;JUSTIFIED;IS TYPEDEF;IS GLOBAL;IS EXTERNAL;INVALID;INDEX;GROUP-USAGE;GLOBAL;FUNCTION-POINTER;FLOAT-SHORT;FLOAT-LONG;FLOAT-EXTENDED;FLOAT-DECIMAL-34;FLOAT-DECIMAL-16;FLOAT-BINARY-64;FLOAT-BINARY-32;FLOAT-BINARY-128;FILLER;EXTERNAL;DYNAMIC;DISPLAY;DESTINATION;DEFAULT;CONSTANT RECORD;COMP-X;COMP-N;COMP-9;COMP-6;COMP-5;COMP-4;COMP-3;COMP-2;COMP-15;COMP-10;COMP-1;COMP-0;COMP;CLASS;BLANK;BIT;BINARY-SHORT;BINARY-LONG;BINARY-DOUBLE;BINARY-C-LONG;BINARY-CHAR;BINARY;BASED;ANY;ALIGNED]
    ...
    ----                 ^
       ...
       8           01 VAR PICTURE X USAGE DISPLAY.
    (line 5, character 14):
    List of completions (75 entries): [VARYING;VALUES;VALUE;VAL-STATUS;USAGE;TYPEDEF;TYPE;TRAILING;SYNCHRONIZED;SIGN;SELECT;SAME;RENAMES;REDEFINES;PROPERTY;PROGRAM-POINTER;PROCEDURE-POINTER;PRESENT;POINTER;PICTURE;.;PACKED-DECIMAL;OCCURS;OBJECT;NATIONAL;LEADING;JUSTIFIED;IS TYPEDEF;IS GLOBAL;IS EXTERNAL;INVALID;INDEX;GROUP-USAGE;GLOBAL;FUNCTION-POINTER;FLOAT-SHORT;FLOAT-LONG;FLOAT-EXTENDED;FLOAT-DECIMAL-34;FLOAT-DECIMAL-16;FLOAT-BINARY-64;FLOAT-BINARY-32;FLOAT-BINARY-128;EXTERNAL;DYNAMIC;DISPLAY;DESTINATION;DEFAULT;CONSTANT RECORD;CONSTANT;COMP-X;COMP-N;COMP-9;COMP-6;COMP-5;COMP-4;COMP-3;COMP-2;COMP-15;COMP-10;COMP-1;COMP-0;COMP;CLASS;BLANK;BIT;BINARY-SHORT;BINARY-LONG;BINARY-DOUBLE;BINARY-C-LONG;BINARY-CHAR;BINARY;BASED;ANY;ALIGNED]
    ...
    ----                            ^
      ...
      10           PROCEDURE DIVISION.
    (line 7, character 25):
    List of completions (75 entries): [VARYING;VALUES;VALUE;VAL-STATUS;USAGE;TYPEDEF;TYPE;TRAILING;SYNCHRONIZED;SIGN;SELECT;SAME;REDEFINES;PROPERTY;PROGRAM-POINTER;PROCEDURE-POINTER;PRESENT;POINTER;PICTURE;.;PACKED-DECIMAL;OCCURS;OBJECT;NATIONAL;LOCALE;LEADING;JUSTIFIED;IS TYPEDEF;IS GLOBAL;IS EXTERNAL;INVALID;INDEX;GROUP-USAGE;GLOBAL;FUNCTION-POINTER;FLOAT-SHORT;FLOAT-LONG;FLOAT-EXTENDED;FLOAT-DECIMAL-34;FLOAT-DECIMAL-16;FLOAT-BINARY-64;FLOAT-BINARY-32;FLOAT-BINARY-128;EXTERNAL;DYNAMIC;DISPLAY;DESTINATION;DEPENDING;DEFAULT;CONSTANT RECORD;COMP-X;COMP-N;COMP-9;COMP-6;COMP-5;COMP-4;COMP-3;COMP-2;COMP-15;COMP-10;COMP-1;COMP-0;COMP;CLASS;BLANK;BIT;BINARY-SHORT;BINARY-LONG;BINARY-DOUBLE;BINARY-C-LONG;BINARY-CHAR;BINARY;BASED;ANY;ALIGNED]
    ...
    ----                                  ^
      ...
      11             DISPLAY DATA-NAME.
    (line 8, character 31):
    List of completions (13 entries): [ZEROS;WHEN;TO;THRU;SPACES;SET;QUOTES;.;LOW-VALUES;IN;HIGH-VALUES;FALSE;ALL] |}];;

let%expect_test "eager-datadiv-completion" =
  let end_with_postproc = completion_positions ~eager:true datadiv in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
    __rootdir__/prog.cob:6.11:
...
let procedure_paragraph = extract_position_markers {cobol|
        IDENTIFICATION DIVISION.
        PROGRAM-ID. prog.
        DATA DIVISION.
        WORKING-STORAGE SECTION.
...
let%expect_test "procedure-paragraph-completion" =
  let end_with_postproc = completion_positions ~eager:false procedure_paragraph in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
...
let%expect_test "eager-procedure-paragraph-completion" =
  let end_with_postproc = completion_positions ~eager:true procedure_paragraph in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
    __rootdir__/prog.cob:9.10:
...
let qualified_data_ref = extract_position_markers {cobol|
        IDENTIFICATION DIVISION.
        PROGRAM-ID. prog.
        DATA DIVISION.
        WORKING-STORAGE SECTION.
...
let%expect_test "qualified-data-ref-completion" =
  let end_with_postproc = completion_positions ~eager:false qualified_data_ref in
  end_with_postproc [%expect.output];
  [%expect {|
...
let%expect_test "eager-qualified-data-ref-completion" =
  let end_with_postproc = completion_positions ~eager:true qualified_data_ref in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[{"message":"Unexpected PICTURE clause for group item 'CC'","range":{"end":{"character":25,"line":6},"start":{"character":16,"line":6}},"severity":1}],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
    __rootdir__/prog.cob:12.18:
...
let procedure = extract_position_markers {cobol|
        IDENTIFICATION DIVISION.
        PROGRAM-ID. prog.
        DATA DIVISION.
        WORKING-STORAGE SECTION.
...
let%expect_test "procedure-completion" =
  let end_with_postproc = completion_positions ~eager:false procedure in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
    __rootdir__/prog.cob:9.10:
    ...
    ----                        ^
      ...
      11             MULTIPLY 4 BY 2 GIVING BB
    (line 8, character 21):
    List of completions (108 entries): [ZEROS;WRITE;WITH NO ADVANCING;WITH;VALIDATE;UPON;UNSTRING;UNLOCK;UNDERLINE;TRANSFORM;TERMINATE;SUPPRESS;SUPER;SUBTRACT;STRING;STOP;START;SPACES;SORT;SIZE;SET;SEND;SELF;SEARCH;REWRITE;REVERSE-VIDEO;RETURN;RESUME;RELEASE;RECEIVE;READ;RAISE;QUOTES;PURGE;POSITION;.;PERFORM;PAGE-COUNTER;OVERLINE;OPEN;ON EXCEPTION;OF;OF SUPER;NULL;NOT ON EXCEPTION;NEXT SENTENCE;MULTIPLY;MOVE;MODE;MERGE;LOW-VALUES;LOWLIGHT;LINE-COUNTER;LINE;LINAGE-COUNTER;LEFTLINE;INVOKE;INSPECT;INITIATE;INITIALIZE;IN;IF;HIGH-VALUES;HIGHLIGHT;GRID;GOBACK;GO;GENERATE;FUNCTION;FREE;FOREGROUND-COLOR;EXIT;EXCEPTION-OBJECT;EXCEPTION;EVALUATE;ERASE;ENTRY;ENTER;END-DISPLAY;ENABLE;DIVIDE;DISPLAY;DISABLE;DELETE;CONTROL;CONTINUE;COMPUTE;COLUMN;COL;CLOSE;CANCEL;CALL;BLINK;BLANK;BELL;BACKGROUND-COLOR;AT;AS;AS UNIVERSAL;AS FACTORY OF;ALTER;ALLOCATE;ALL;ADDRESS;ADD;ACCEPT;AA;BB]
...
let%expect_test "eager-procedure-completion" =
  let end_with_postproc = completion_positions ~eager:true procedure in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
    __rootdir__/prog.cob:9.10:
...
let control = extract_position_markers {cobol|
        CONTROL DIVISION.
...
let%expect_test "control-completion" =
  let end_with_postproc = completion_positions ~eager:false control  in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
    ...
    (line 3, character 10):
    List of completions (9 entries): [PROGRAM-ID;.;INTERFACE-ID;IDENTIFICATION;ID;FUNCTION-ID;DISPLAY;CLASS-ID;ACCEPT]
...
let%expect_test "eager-control-completion" =
  let end_with_postproc = completion_positions ~eager:true control  in
  end_with_postproc [%expect.output];
  [%expect {|
    {"params":{"diagnostics":[],"uri":"file://__rootdir__/prog.cob"},"method":"textDocument/publishDiagnostics","jsonrpc":"2.0"}
    __rootdir__/prog.cob:3.8:
