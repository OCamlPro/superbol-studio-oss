...
let%expect_test "context-sensitive-tokens" =
  ...
  [%expect {|
    ...
    WITH NO ADVANCING, END-DISPLAY, ., STOP, RUN, ., EOF
|}];;
...
let%expect_test "context-sensitive-tokens-ter" =
  ...
  [%expect {|
    ...
    PICTURE, PICTURE_STRING[9], VALUE, DIGITS[0], ., DIGITS[01],
    WORD[BYTE-LENGTH], PICTURE, PICTURE_STRING[9], ., DIGITS[01], WORD[X],
    CONSTANT, IS GLOBAL, AS, BYTE-LENGTH, WORD[Y], ., DIGITS[01], WORD[Y],
...
let%expect_test "context-sensitive-tokens-with-syntax-errors" =
  ...
  [%expect {|
    ...
    DISPLAY, WORD[X], WORD[AWAY-FROM-ZERO], WITH NO ADVANCING, END-DISPLAY, .,
    STOP, RUN, ., EOF
|}];;
...
let%expect_test "context-sensitive-tokens-weird-exit" =
  ...
  [%expect {|
    ...
    ., WORKING-STORAGE, SECTION, ., DIGITS[01], WORD[X], CONSTANT, IS GLOBAL,
