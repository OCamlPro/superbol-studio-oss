...
let preproc ~filename
    ?(sql_in_copybooks = false)
    ?(copy_path = [])
    ?(copy_exts = [])
    ?(contents = EzFile.read_file filename)
    ~source_format () =
  ...
  let copy_exts = match copy_exts with
    | [] -> [ ".cpy" ]
    | _ -> copy_exts
  in
  let copy_path = Filename.dirname filename :: copy_path in
  let copy_path = lazy ( List.map (fun dir ->
  ...
  let config = { scanner_config ;
                 ...
                 copy_path ;
                 copy_exts ;
                 verbosity = scanner_config.verbosity } in

  let sql_statements = Parse.parse ~config ~filename ~contents in

  let contents = Generate.generate ~filename ~contents sql_statements in
