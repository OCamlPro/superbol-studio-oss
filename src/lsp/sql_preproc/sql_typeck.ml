...
let get_x_info (cu:Cobol_unit.Types.cobol_unit) name_str = 
  ...
  with 
  | Not_found -> Pretty.out " \"%s\" not found " name_str; failwith "Var not found"
  | Cobol_unit.Qualmap.Ambiguous _ -> Pretty.out " \"%s\" not found. qualname nel lazy_t found" name_str; failwith "Var not found"



let get_size cu name =
  let x_info = get_x_info cu name
  in
  match x_info with
...
let print_name (cu:Cobol_unit.Types.cobol_unit) =
  let x_info = get_x_info cu "VBFLD"
  in
  match x_info with
