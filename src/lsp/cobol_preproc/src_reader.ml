...
let make make_lexing ?tab_width ?filename ~source_format ~platform input =
  ...
  Plx (Src_lexing.init_state ?tab_width source_format, lexbuf, platform)
...
let from ?source_format ?tab_width ~platform (input: Src_input.t) =
  let source_format = decide_on_source_format ~platform ?source_format input in
  match input with
  | { source = String contents; filename } ->
      from_string ?tab_width ~source_format ~filename ~platform contents
  | { source = Channel ic; filename } ->
      from_channel ?tab_width ~source_format ~filename ~platform ic
...
let restart make_lexing make_input ?tab_width ?source_format ?position
  ...
  | Some _ | None ->
      from ?source_format ?tab_width ~platform @@
