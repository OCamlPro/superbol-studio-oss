...
(** [fold_source_lines ~dialect ~source_format ~skip_compiler_directives_text
    ...
    When given, [on_source_format_change] is called each time the source format
    changes: once at the begining {e before} [f] has been called, and then once
    each time a compiler directive that changes the source format right after
    the text is fed to [f] but before [on_compiler_directive] is called.
...
val fold_source_lines
  : dialect: Cobol_config.dialect
  -> source_format: Cobol_config.source_format_spec
  -> ?on_source_format_change: (Src_format.any -> 'a -> 'a)
