...
let contexts_for_state_num: int -> _ list = function
  ...
  | 1482 -> [constant]
  | 1949 -> [occurs_clause]
  | 1976 -> [line_clause                             (*NUMBERS only*)]
  | 1978 -> [line_clause                             (*NUMBERS only*)]
  | 1989 -> [column_clause              (* NUMBERS & CENTER *)]
  | 1995 -> [column_clause              (* NUMBERS & CENTER *)]
  | 2000 -> [column_clause              (* NUMBERS & CENTER *)]
  | 2022 -> [rounded_phrase]
  | 2091 -> [screen_descr_entry]
  | 2132 -> [erase_clause]
  | 2389 -> [retry_phrase]
  | 2483 -> [stop_stmt]
  | 2570 -> [set_stmt]
  | 2630 -> [set_attribute_stmt]
  | 2718 -> [resume_stmt]
  | 2736 -> [read_stmt]
  | 2795 -> [sharing_phrase]
  | 3040 -> [exit_stmt]
  | 3177 -> [allocate_stmt]
  | 3191 -> [accept_stmt]
  | 3906 -> [program_id_paragraph]
  | 4036 -> [object_paragraph]
  | 4046 -> [factory_paragraph]
  | _ -> []
