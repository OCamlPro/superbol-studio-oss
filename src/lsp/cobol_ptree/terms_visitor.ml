  ...
  handle v#fold_condition
    ~continue:begin fun (c: condition) x -> match c with
      | CondSubject (e, c) -> x
          ...
          >> fold_condition' v c
      | CondRelOp (r, c) -> x
          >> fold_relop v r.payload
          >> fold_condition' v c
      | CondObjectOrExpr e -> x
          >> fold_expr' v e
      | CondParen c
      | CondNot c -> x
          >> fold_condition' v c
      | CondCombined (c, l, d) -> x
          >> fold_condition' v c
          >> fold_logop v l
          >> fold_condition' v d
      | CondClass c -> x
