...
class virtual ['a] folder = object
  ...
  method fold_stop_run_body       : (stop_run_body           , 'a) fold = default
...
let fold_stop_run_body (v: _ #folder) =
  handle v#fold_stop_run_body
    ~continue:begin fun r x -> match r with
      | StopReturnScalar s -> x
        >> fold_scalar v s
      | StopReturnAddress a -> x
        >> fold_qualident v a
      | StopReturnInt { value; size } -> x
        >> fold_literal v value
        >> fold_option ~fold:fold_literal v size
      | StopReturnStatus { status_kind = _st; status_value} -> x
        >> fold_option ~fold:fold_scalar v status_value
    end
...
let fold_stop' (v: _ #folder) =
  handle' v#fold_stop' v
    ~fold:begin fun v -> function
      ...
      | StopRun r -> fold_option ~fold:fold_stop_run_body v r
      | StopError -> Fun.id
    end
