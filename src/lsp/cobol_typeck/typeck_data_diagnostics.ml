...
type error =
  ...
  | Unexpected_redefinition_name of
      {
        redef_loc: srcloc;
        redef_name: Cobol_ptree.Types.data_name with_loc option;
        redef_redefines: Cobol_ptree.Types.name with_loc;
  ...
  | Unexpected_picture_clause of
      {
        picture: Cobol_ptree.Types.picture_clause with_loc;
        reason: [`Group_item |
                 `Item_with_usage of Cobol_ptree.Types.usage_clause ];
        item_name: Cobol_ptree.Types.data_name with_loc option;
        item_loc: srcloc;
      }
  ...
  | Occurs_in_rename_operand of
      {
        operand: Cobol_ptree.Types.qualname with_loc;
  ...
  | Invalid_renaming_of_variable_length_range of
      {
        loc: srcloc;
        depending_vars: Cobol_ptree.Types.qualname NEL.t;
      }
  | Picture_error of
      {
        picture_loc: srcloc;
  ...
  | Incompatible_picture of
      {
        ...
        usage_clause: Cobol_ptree.Types.usage_clause;
        expected: [`Numeric_category | `Boolean_class | `Nonalpha_class];
      }
  | Item_not_found of
      {
        qualname: Cobol_ptree.Types.qualname with_loc;
      }
  | Misplaced of
      {
        entry: entry;
  ...
  | Redefinition_of_table_item of     (* in GnuCOBOL that's a warning as well *)
      {
        ...
        redef_name: Cobol_ptree.Types.data_name with_loc option;
        redef_redefines: Cobol_ptree.Types.name with_loc;
        table_item_name: Cobol_ptree.Types.qualname with_loc option;
        (* table_item: (\* ([>`table], _)  *\)Cobol_data.Types.item_definition with_loc; *)
      }
  | Mismatching_usage_in_group of                 (* not enforced by GnuCOBOL *)
      {
        item_name: Cobol_ptree.Types.data_name with_loc option;
        item_usage: Cobol_ptree.Types.usage_clause with_loc;
        group_usage: Cobol_ptree.Types.usage_clause with_loc;
      }
  | Duplicate_clause of
      {
        first_loc: srcloc;
...
let pp_data_name'_opt
  : Cobol_ptree.Types.data_name with_loc option Pretty.printer
  = fun ppf -> function
...
let pp_error ppf = function
  | Incompatible_picture { picture; usage_clause; expected } ->
      Pretty.print ppf "PICTURE@ of@ category@ %a@ is@ incompatible@ with@ \
        ...
        Cobol_ptree.Types.pp_usage_clause usage_clause
        (match expected with
  ...
  | Unexpected_picture_clause { item_name; reason = `Item_with_usage u; _ } ->
      Pretty.print ppf "Unexpected@ PICTURE@ clause@ for@ %a@ with@ USAGE@ %a"
        pp_data_name'_opt item_name Cobol_ptree.Types.pp_usage_clause u
  ...
  | Occurs_in_rename_operand { operand; _ } ->
      Pretty.print ppf "RENAMES@ operand@ '%a'@ has@ or@ is@ subordinate@ to@ \
                        an@ OCCURS@ clause"
        Cobol_ptree.Types.pp_qualname' operand
  | Invalid_renaming_range { from_field; thru_field; _ } ->
      Pretty.print ppf "Invalid@ pair@ of@ RENAMES@ operands: first@ field@ %a@ \
        ...
        (Fmt.option Cobol_ptree.Types.pp_qualname') ~&from_field.field_qualname
        (Fmt.option Cobol_ptree.Types.pp_qualname') ~&thru_field.field_qualname
  | Invalid_renaming_of_variable_length_range { depending_vars = vars; _ } ->
      Pretty.print ppf "Renaming@ of@ variable-length@ range@ (length@ depends@ \
                        on:@ %a)"
        Fmt.(list ~sep:comma Cobol_ptree.Types.pp_qualname) (NEL.to_list vars)
  | Picture_error { error; _ } ->
      Cobol_data.Picture.pp_error ppf ~&error
  | Item_not_found { qualname; _ } ->
      Pretty.print ppf "Item@ '%a'@ not@ found" Cobol_ptree.Types.pp_qualname' qualname
...
let pp_warning ppf = function
  | Redefinition_of_table_item { table_item_name; _ } ->
      Pretty.print ppf "Redefinition@ of@ item@ with@ OCCURS@ clause%a"
        Fmt.(option (sp ++ Cobol_ptree.Types.pp_qualname')) table_item_name
  | Mismatching_usage_in_group { item_usage; item_name; group_usage } ->
      Pretty.print ppf "Mismatching@ USAGE@ %a@ for@ %a,@ subordinate@ to@ a@ \
                        group@ with@ USAGE@ %a"
        Cobol_ptree.Types.pp_usage_clause ~&item_usage
        pp_data_name'_opt item_name
        Cobol_ptree.Types.pp_usage_clause ~&group_usage
